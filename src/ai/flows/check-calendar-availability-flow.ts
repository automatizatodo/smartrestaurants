...
const checkCalendarAvailabilityFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    // console.log('CALENDAR_CHECK_AVAIL_FLOW_ENV_CHECK: GOOGLE_CREDENTIALS_JSON set:', !!process.env.GOOGLE_CREDENTIALS_JSON);
    ...
    if (googleCredentialsJson) {
      // console.log('CALENDAR_CHECK_AVAIL_FLOW: Found GOOGLE_CREDENTIALS_JSON, attempting to use it.');
      try {
        authOptions.credentials = JSON.parse(googleCredentialsJson);
      } catch (e: any) {
        console.error('CALENDAR_CHECK_AVAIL_FLOW: CRITICAL - Failed to parse GOOGLE_CREDENTIALS_JSON:', e.message);
        return { isAvailable: false, reasonKey: 'landing:booking.error.calendarConfigError' };
      }
    } else if (googleAppCredentialsPath) {
      // console.log('CALENDAR_CHECK_AVAIL_FLOW: GOOGLE_CREDENTIALS_JSON not found, using GOOGLE_APPLICATION_CREDENTIALS path:', googleAppCredentialsPath);
      authOptions.keyFile = googleAppCredentialsPath;
    } else {
      console.error('CALENDAR_CHECK_AVAIL_FLOW: CRITICAL - Neither GOOGLE_CREDENTIALS_JSON nor GOOGLE_APPLICATION_CREDENTIALS are set.');
      return { isAvailable: false, reasonKey: 'landing:booking.error.calendarConfigError' };
    }
