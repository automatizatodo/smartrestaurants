...
import type { PriceSummary } from '@/app/api/menu/route'; // Import PriceSummary
...
interface MenuServiceResponse {
  ...
  priceSummary: PriceSummary; // Add priceSummary to the response
}
...
export async function fetchMenuDataWithPrice(): Promise<MenuServiceResponse> {
  ...
  try {
    ...
    if (itemCount === 0 && !result.currentMenuPrice && (!result.priceSummary || Object.keys(result.priceSummary).length === 0)) {
      ...
      return { menuItems: [], currentMenuPrice: result.currentMenuPrice, priceSummary: result.priceSummary || {} }; // Ensure we always return an array for menuItems and a PriceSummary object
    }
    return result;
